        ...
        {rows.map(({ d, count }) => (
          <article key={d.slug} className="card card-hover overflow-hidden">
            <Link href={`/dealer/${d.slug}`} className="block">
              ...
              >
                <span
                  className="absolute -bottom-6 right-5 grid h-14 w-14 place-items-center rounded-2xl border-2 text-xl font-extrabold"
                  style={{ background: "var(--surface-1)", borderColor: "var(--surface-1)", color: "var(--brand)" }}
                >
                  {d.name.trim().slice(0, 1)}
                </span>
                {d.verified && (
                  <span className="tag absolute top-3 left-3" style={{ background: "rgba(255,255,255,0.16)", color: "#fff" }}>
