...
export interface CreditResult {
  ...
  totalPaid: number;
  cost: number;
  /** نسبة الجهد الشهري مقارنة بالدخل */
  effort?: number;
}

export function computeCredit(input: CreditInput, income?: number): CreditResult {
  const financed = Math.max(0, input.price - input.downPayment);
  let monthly: number;
  if (input.type === "mourabaha") {
    // مرابحة: هامش ربح ثابت يُوزّع على المدة
    const margin = financed * (input.rate / 100) * (input.months / 12);
    monthly = (financed + margin) / input.months;
  } else {
    const i = input.rate / 100 / 12;
    monthly = i === 0
      ? financed / input.months
      : (financed * i) / (1 - Math.pow(1 + i, -input.months));
  }
  const totalPaid = monthly * input.months;
  return {
    financed: Math.round(financed),
    monthly: Math.round(monthly),
    totalPaid: Math.round(totalPaid),
    cost: Math.round(totalPaid - financed),
    effort: income ? monthly / income : undefined,
  };
}
