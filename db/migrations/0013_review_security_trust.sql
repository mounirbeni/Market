...
-- Inspection used to be seller-controlled. Archive the old value first, clear
-- the public badge, then restore only a latest successful admin approval logged
-- by the short-lived base-branch moderation action. This avoids erasing a real
-- approval if that action was used before this migration ships.
UPDATE listings SET seller_inspection_claimed = inspected, inspected = false;

WITH latest_admin_inspection AS (
  SELECT DISTINCT ON (target) target AS ref, action
    FROM admin_log
   WHERE target IS NOT NULL
     AND action IN ('listing:inspect', 'listing:uninspect')
   ORDER BY target, created_at DESC, id DESC
)
UPDATE listings l
   SET inspected = (a.action = 'listing:inspect'),
       seller_inspection_claimed = CASE
         WHEN a.action = 'listing:inspect' THEN false
         ELSE l.seller_inspection_claimed
       END
  FROM latest_admin_inspection a
 WHERE l.ref = a.ref;

-- The original seller checkbox generated the fixed sentinel dates below.
-- Base commit df9c62a (deployed 2026-09-09) briefly generated relative dates
-- instead: +12 months for "valid" and -1 month for "invalid". Until this
-- migration is applied there is still no seller UI for a real expiry date, so
-- every value in those generated windows is synthetic and must be archived.
UPDATE listings
   SET legacy_technical_control = technical_control,
       technical_control = NULL
 WHERE technical_control IN (DATE '2026-01-01', DATE '2027-01-01')
    OR technical_control BETWEEN DATE '2027-09-09'
                             AND (((now() AT TIME ZONE 'UTC')::date + interval '12 months')::date)
    OR technical_control BETWEEN DATE '2026-08-09'
                             AND (((now() AT TIME ZONE 'UTC')::date - interval '1 month')::date);
